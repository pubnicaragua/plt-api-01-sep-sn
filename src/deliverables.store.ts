...
const ROLE_SEED: SeedDeliverable[] = [
  { id: 'role-admin', title: 'Rol 01 · Administrador General', area: 'Roles y permisos', summary: 'Usuarios, roles, clientes, conductores, vehículos, solicitudes, pagos, incidencias, tarifas, reportes y auditoría.', status: 'in_progress', priority: 'Alta', evidence: 'Rol implementado en UsersStore con permiso global (*) y gestión desde el panel.', source: 'En implementación' },
  { id: 'role-management', title: 'Rol 02 · Gerencia', area: 'Roles y permisos', summary: 'Consulta y exportación de dashboard, operación, costos, pagos, incidencias y reportes sin mutaciones sensibles.', status: 'in_progress', priority: 'Alta', evidence: 'Permisos de consulta y exportación definidos en la matriz del panel.', source: 'En implementación' },
  { id: 'role-operations', title: 'Rol 03 · Operaciones o Despacho', area: 'Roles y permisos', summary: 'Solicitudes, validación operativa, asignaciones, tracking, estados, retrasos y cancelaciones autorizadas.', status: 'in_progress', priority: 'Alta', evidence: 'Permisos de despacho definidos; asignación y transiciones de estado operativas en la API.', source: 'En implementación' },
  { id: 'role-finance', title: 'Rol 04 · Finanzas o Caja', area: 'Roles y permisos', summary: 'Montos, saldos, pagos, parciales, comprobantes, crédito, contra factura y reportes financieros.', status: 'in_progress', priority: 'Alta', evidence: 'Permisos financieros definidos; módulo de facturas protegido con acceso de Mario Martínez.', source: 'En implementación' },
  { id: 'role-support', title: 'Rol 05 · Soporte', area: 'Roles y permisos', summary: 'Casos, mensajes, adjuntos, solicitudes de evidencia, escalamiento, resolución y cierre.', status: 'in_progress', priority: 'Alta', evidence: 'Permisos de soporte definidos en la matriz; canal de chat pendiente.', source: 'En implementación' },
  { id: 'role-driver', title: 'Rol 06 · Conductor', area: 'Roles y permisos', summary: 'Servicios disponibles y asignados, estados, ubicación, incidencias, evidencias y confirmación de entrega.', status: 'in_progress', priority: 'Alta', evidence: 'Permisos de conductor definidos; usuarios conductores operativos en UsersStore.', source: 'En implementación' },
  { id: 'role-corporate-user', title: 'Rol 07 · Usuario Corporativo', area: 'Roles y permisos', summary: 'Solicitudes, direcciones, cotizaciones, pagos reportados, tracking, comprobantes y soporte de su empresa.', status: 'in_progress', priority: 'Alta', evidence: 'Permisos corporativos definidos en la matriz; portal de empresa pendiente.', source: 'En implementación' },
  { id: 'role-store', title: 'Rol 08 · Tienda o Recepción', area: 'Roles y permisos', summary: 'Recepciones, validación física, peso, dimensiones, fotografías, observaciones e historial de carga.', status: 'in_progress', priority: 'Alta', evidence: 'Permisos de validación de carga definidos en la matriz; bandeja de recepción pendiente.', source: 'En implementación' },
]
